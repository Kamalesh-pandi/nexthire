    ...
    <div className="space-y-8 pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-extrabold text-slate-100 mt-1">Curriculum Improvements & Projects</h1>
        </div>

        <button
          onClick={() => setIsProjectModalOpen(true)}
          className="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs shadow-lg shadow-emerald-500/20 flex items-center gap-2"
        >
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2">
          <BookOpenCheck className="w-5 h-5 text-emerald-400" /> Co-Sponsored Industry Capstones
        </h3>

        {projectsLoading ? (
          <div className="glass-panel rounded-3xl p-8 text-center text-slate-400 text-xs">
            Fetching capstone projects from Firestore database...
          </div>
        ) : projects.length === 0 ? (
          <div className="glass-panel rounded-3xl p-8 text-center text-slate-400 text-xs">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {projects.map((proj) => (
              <div key={proj.id} className="glass-card rounded-3xl p-6 border border-slate-700/60 space-y-4">
                <div className="flex items-center justify-between">
                ...
                <div className="space-y-1">
                  <span className="text-[11px] text-slate-400">Required Skill Prerequisites:</span>
